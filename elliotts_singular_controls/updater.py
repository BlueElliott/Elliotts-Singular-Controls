"""Auto-update functionality for Elliott's Singular Controls."""
...
GITHUB_REPO = "BlueElliott/Elliotts-Singular-Controls"
...
def check_for_updates() -> Optional[Dict]:
    ...
    Returns:
        Dict with keys: version, download_url, release_notes, asset_name
...
def download_update(download_url: str, asset_name: str) -> Optional[Path]:
    ...
    try:
        ...
        temp_dir = Path(tempfile.gettempdir()) / "singular_controls_updates"
...
def install_update(new_exe_path: Path) -> bool:
    ...
    try:
        ...
        script_path = current_exe.parent / "update_esc.bat"

        # Batch script to replace exe and restart
        script_content = f"""@echo off
echo Updating Elliott's Singular Controls...
